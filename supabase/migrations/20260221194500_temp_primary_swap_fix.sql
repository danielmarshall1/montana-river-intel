...
begin
  ...
  inserted as (
    insert into public.river_usgs_map_suggestions (
      river_id, role, site_no, candidate_rank, rank_score, distance_to_river_m,
      on_river_alignment, name_match, has_temp_iv, has_temp_dv, has_flow, has_stage,
      selection_reason, suggestion_status, suggested_at
    )
    select
      r.river_id, 'temp', r.site_no, r.candidate_rank, r.rank_score, r.distance_to_river_m,
      r.on_river_alignment, r.name_match, r.has_temp_iv, r.has_temp_dv, r.has_flow, r.has_stage,
      case
  ...
  if p_auto_apply then
    create temporary table if not exists _mri_best_temp_sites
    (river_id uuid primary key, site_no text not null) on commit drop;

    truncate _mri_best_temp_sites;

    insert into _mri_best_temp_sites (river_id, site_no)
    ...
    -- Demote existing active primary temp mapping before promoting chosen row.
    update public.river_usgs_map_roles m
    set
      is_active = false,
      priority = greatest(2, m.priority),
      updated_at = now()
    where m.role = 'temp'
      ...
      and exists (
        select 1
        from _mri_best_temp_sites b
        where b.river_id = m.river_id
      );

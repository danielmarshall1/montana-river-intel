-- ─────────────────────────────────────────────────────────────────────────────
-- Rolling 3-day precip clarity penalty
--
-- Replaces single-day precip_mm in the runoff/clarity penalty with a
-- 3-day rolling sum so cumulative wet spells are penalized correctly.
--
-- Changes:
--   1. pre CTE: adds rolling_precip_3d subquery
--   2. penalties lateral: old precip bands → new rolling bands + keep
--      single-day >=15mm spike for heavy single events
--   3. Step 2 fallback: same rolling logic (uses same subquery inline)
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- penalties + modifiers (tailwater-aware, rolling precip)
  join lateral (
    select
      -- wind penalty
      case
      ...
      -- single-day precip penalty (kept for heavy spike events)
      case
      ...
      least(30::numeric, (
        ...
        -- 3-day rolling precip: replaces old single-day bands
        ...
        -- heavy single-day spike: additional penalty

-- Temperature diagnostics hardening:
-- - strict same-river temp trust indicators
-- - freshness status fields in latest/detail views
...
select
  ...
  coalesce(r.source_payload->>'temp_site_no', r.usgs_site_no) as temp_source_site_no,
  case
...
select
  ...
  case
    when ld.source_temp_observed_at is null then true
    when extract(epoch from (now() - ld.source_temp_observed_at)) > (6 * 3600) then true
    else false
  end as temperature_stale
from public.rivers r
left join latest_daily ld on ld.river_id = r.id

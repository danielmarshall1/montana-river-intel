-- Rewrite compute_river_daily_scores Step 0 to only nullify invalid temp_source_site_no values.
--
-- Previous Step 0 blindly overwrote temp_source_site_no with the priority-1 station from
-- river_usgs_map_roles on every run, clobbering the ingest's authoritative selection of
-- whichever station actually returned live data (which may be a valid priority-2 fallback).
--
-- New Step 0: only NULLs out temp_source_site_no when the current value is no longer an
-- active temp role for that river (e.g. after removing a bad/wrong station assignment).
-- The ingest remains authoritative for writing which station delivered data.
...
as $$
begin
  -- ── Step 0: nullify temp_source_site_no when it no longer maps to an active temp role ──
  -- Only clears stale/wrong assignments — does NOT overwrite valid ingest-selected stations.
  update public.river_daily d
  set temp_source_site_no = null
  where (p_obs_date is null or d.obs_date = p_obs_date)
    and (p_river_id is null or d.river_id = p_river_id)
    and d.temp_source_site_no is not null
    and not exists (
      select 1
      from public.river_usgs_map_roles m
      where m.river_id = d.river_id
        and m.site_no  = d.temp_source_site_no
        and m.role     = 'temp'

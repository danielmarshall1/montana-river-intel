-- Fix duplicate rows in v_river_detail_analytics caused by flow_source_names and
-- temp_source_names CTEs joining both usgs_sites and usgs_station_registry.
-- If a site_no exists in both tables the LEFT JOIN produces 2 rows per river.
-- Fix: use scalar subqueries instead of double LEFT JOIN.
...
select
  ...
  -- Use scalar subqueries to avoid fanout when site_no exists in both usgs_sites and usgs_station_registry
  coalesce(
    (select us.station_name from public.usgs_sites us where us.site_no = b.flow_source_site_no limit 1),
    (select reg.station_name from public.usgs_station_registry reg where reg.site_no = b.flow_source_site_no limit 1)
  ) as flow_source_site_name,
  b.temp_source_site_no,
  coalesce(
    (select us.station_name from public.usgs_sites us where us.site_no = b.temp_source_site_no limit 1),
    (select reg.station_name from public.usgs_station_registry reg where reg.site_no = b.temp_source_site_no limit 1)
  ) as temp_source_site_name,

-- ─────────────────────────────────────────────────────────────────────────────
-- Tailwater exception for scoring:
--   1. Add is_tailwater boolean to rivers table
--   2. Mark Bighorn and Kootenai (below Libby Dam) as tailwaters
--   3. Update compute_river_daily_scores to:
--        - Skip month-based runoff penalty baseline for tailwaters
--        - Skip flow_ratio turbidity penalty for tailwaters
--        - Apply tailwater-specific seasonal multipliers (spring advantage)
--        - Still apply thermal modifier, wind, precip, and rising-flow penalties
-- ─────────────────────────────────────────────────────────────────────────────

alter table public.rivers
  add column if not exists is_tailwater boolean default false;

update public.rivers
set is_tailwater = true
where slug in ('bighorn-st-xavier', 'kootenai-below-libby-dam');

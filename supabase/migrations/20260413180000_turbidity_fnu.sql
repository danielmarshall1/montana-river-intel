-- ─────────────────────────────────────────────────────────────────────────────
-- Turbidity (FNU) support
--
-- Adds river_daily.turbidity_fnu column and wires it into scoring.
--
-- USGS parameter 63680 (Formazin Nephelometric Units) is direct water clarity
-- measurement. When present it supersedes the flow/precip clarity estimates.
--
-- Active sites in our system today:
--   12340500 Clark Fork above Missoula  → 10.4 FNU (fresh, mapped to clark-fork-st-regis)
--   12324200 Clark Fork at Deer Lodge   →  5.8 FNU (fresh, mapped to clark-fork-st-regis)
--   12301933 Kootenai below Libby Dam   →  2.3 FNU (last reading 2023, sensor retired)
--
-- Scoring change: when turbidity_fnu IS NOT NULL:
--   < 10 FNU  : no turbidity penalty (clear)
--   10–25 FNU : +5 pts
--   25–50 FNU : +12 pts
--   > 50 FNU  : +20 pts
--   Flow-ratio and rolling-precip clarity bands scaled to 50% weight
--   (direct measurement outweighs indirect proxies).
-- ─────────────────────────────────────────────────────────────────────────────

alter table public.river_daily
  add column if not exists turbidity_fnu float8 default null;

comment on column public.river_daily.turbidity_fnu is
  'Water turbidity in FNU (Formazin Nephelometric Units) from USGS parameter 63680. Null when gauge does not measure turbidity.';
...
begin
  ...
  join lateral (
    select
      ...
      -- overrides/reduces indirect clarity proxies
      case
      ...
      -- (measured FNU outweighs inferred clarity from flow/precip)
      ...
      least(30::numeric, (
        ...
        -- flow-ratio turbidity: scaled down when direct measurement available
        ...
        -- 3-day rolling precip: scaled down when direct measurement available
        ...
        -- heavy single-day spike: scaled down when direct measurement available
